...
RoutePlanner::RoutePlanner(RouteModel &model, float start_x, float start_y, float end_x, float end_y) : m_Model(model) {
    ...
    // TODO 2: Use the m_Model.FindClosestNode method to find the closest nodes to the 
...
float RoutePlanner::CalculateHValue(RouteModel::Node const *node) {
    ...
    // Get the distance to the enqon d_node:
    // node -> distance() equivalent to (*node).distance();
    // Access a method of a pointer using the arrow operator.
    // https://stackoverflow.com/questions/2575048/arrow-operator-usage-in-c
    // RoutePlanner has a private attribute end_node.
...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
    ...
    // Populate the vector of neighbor nodes of the current node
    current_node->FindNeighbors();

    // For each neighbor, set parent to current_node, h_value to distance
    // to end_node, g_value to current_node g + distance to current_node.
...
// TODO: Make a function that compares two pointers to nodes.
// Declaration in .hpp, Definition here.
...
// - When the search has reached the end_node, use the ConstructFinalPath method to return the final 
//   path that was found.
// - Store the final path in the m_Model.path attribute before the method exits. This path will then
